const admin = require('firebase-admin');

const serviceAccount = {
    projectId: "hecho-srl-free",
    clientEmail: "firebase-adminsdk-fbsvc@hecho-srl-free.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\ndesm+luFp0GiszZgkCxk+dad\n-----END PRIVATE KEY-----\n"
};

admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
});

const db = admin.firestore();
const ticketId = "CfMm2xbeGitHQiKcFfom";

async function inspect() {
    console.log("Starting inspect...");
    try {
        const snap = await db.collection("tickets").doc(ticketId).get();
        if (!snap.exists) {
            console.log("Ticket not found.");
            process.exit(0);
        }
        const data = snap.data();
        console.log("=== TICKET DATA ===");
        console.log("ticketNumber:", data.ticketNumber);
        console.log("status:", data.status);
        console.log("arrivedAt:", data.arrivedAt ? (data.arrivedAt.toDate ? data.arrivedAt.toDate().toISOString() : JSON.stringify(data.arrivedAt)) : "null");
        console.log("workStartedAt:", data.workStartedAt ? (data.workStartedAt.toDate ? data.workStartedAt.toDate().toISOString() : JSON.stringify(data.workStartedAt)) : "null");
        console.log("technicianId:", data.technicianId);
        console.log("technicianName:", data.technicianName);
        console.log("visits:", JSON.stringify(data.visits, null, 2));
    } catch (e) {
        console.error("Error inspecting:", e);
    }
    process.exit(0);
}

inspect();
