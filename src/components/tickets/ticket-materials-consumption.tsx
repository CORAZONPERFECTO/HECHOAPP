...
export function TicketMaterialsConsumption({ ticketId, ticketNumber, currentUserRole }: TicketMaterialsConsumptionProps) {
    ...
    const [vehicleStock, setVehicleStock] = useState<InventoryStock[]>([]);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            let defaultLoc = "";
            let assignedLoc: InventoryLocation | undefined;
            if (userId && lData.length > 0) {
                assignedLoc = lData.find(l => l.responsibleUserId === userId);
                if (assignedLoc) {
                    defaultLoc = assignedLoc.id;
                } else if (currentUserRole !== 'TECNICO' && lData.length > 0) {
                    defaultLoc = lData[0].id;
                }
            } else if (lData.length > 0 && currentUserRole !== 'TECNICO') {
                defaultLoc = lData[0].id;
            }
            setSelectedLocation(defaultLoc);

            // If technician, fetch assigned vehicle stock
            if (currentUserRole === 'TECNICO' && defaultLoc) {
                const stockData = await getStockByLocation(defaultLoc);
                setVehicleStock(stockData);
            }
    ...
    const handleAddMaterial = async () => {
        ...
        // Client side validation for technicians
        if (currentUserRole === 'TECNICO' && mode === 'CONSUME') {
            const stockQty = getProductStockQty(selectedProduct);
            if (quantity > stockQty) {
                alert(`No puedes consumir más de lo disponible en tu vehículo. Disponible: ${stockQty}`);
                return;
            }
        }
    ...
    // Derived State: Filtered products for technician selection (only items with stock > 0 in vehicle)
    const filteredProducts = products.filter(p => {
        if (currentUserRole === 'TECNICO') {
            const stockItem = vehicleStock.find(s => s.productId === p.id);
            return stockItem && stockItem.quantity > 0;
        }
        return true;
    });

    const getProductStockQty = (productId: string) => {
        const stockItem = vehicleStock.find(s => s.productId === productId);
        return stockItem ? stockItem.quantity : 0;
    };
        ...
        <div className="space-y-6">
            <Card>
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (userId && locations.length > 0) {
                                        ...
                                        } else if (currentUserRole !== 'TECNICO' && locations.length > 0) {
                                            defaultLoc = locations[0].id;
                                        }
                                    } else if (locations.length > 0 && currentUserRole !== 'TECNICO') {
                                        defaultLoc = locations[0].id;
                                    }
                                    setSelectedLocation(defaultLoc);
                                }}
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (currentUserRole === 'TECNICO') {
                                        ...
                                        const userId = auth.currentUser?.uid;
                                        const assigned = locations.find(l => l.responsibleUserId === userId);
                                        if (assigned) setSelectedLocation(assigned.id);
                                    } else {
                ...
                <CardContent className="space-y-6">
                    {currentUserRole === 'TECNICO' && !selectedLocation ? (
                        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-sm flex items-start gap-2">
                            ...
                            <div>
                                <h4 className="font-semibold">Sin vehículo asignado</h4>
                                <p>No tienes un vehículo o almacén asignado a tu cuenta. Por favor solicita a un administrador que asigne tu usuario a un vehículo en el módulo de Inventario para poder registrar materiales.</p>
                            </div>
                        </div>
                    ) : (
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                            ...
                                            {filteredProducts.map(p => {
                                                ...
                                                return (
                                                    <SelectItem key={p.id} value={p.id}>
                                                        {p.name} {currentUserRole === 'TECNICO' ? `(Disp: ${stockQty} ${p.unit})` : `(${p.unit})`}
                                                    </SelectItem>
                                                );
                                            })}
                                        </SelectContent>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {currentUserRole === 'TECNICO' ? (
                                        <div className="h-10 px-3 py-2 rounded-md border border-input bg-gray-100 text-sm flex items-center text-gray-700 font-medium">
                                            {locations.find(l => l.id === selectedLocation)?.name || "Cargando vehículo..."}
                                        </div>
                                    ) : (
