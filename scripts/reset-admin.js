const admin = require('firebase-admin');

// Hardcoded config for emergency reset
const serviceAccount = {
    projectId: "hecho-srl-free",
    clientEmail: "firebase-adminsdk-fbsvc@hecho-srl-free.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\ndesm+luFp0GiszZgkCxk+dad\n-----END PRIVATE KEY-----\n"
};

admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
});

const TARGET_EMAIL = "lcaa27@gmail.com";
const NEW_PASSWORD = "Password123!"; // <--- Nueva contraseña

async function resetPassword() {
    try {
        console.log(`Buscando usuario: ${TARGET_EMAIL}...`);
        const userRecord = await admin.auth().getUserByEmail(TARGET_EMAIL);
        
        console.log(`Usuario encontrado. ID: ${userRecord.uid}. Forzando nueva contraseña...`);
        await admin.auth().updateUser(userRecord.uid, {
            password: NEW_PASSWORD
        });

        console.log(`\n✅ ¡ÉXITO! La contraseña para ${TARGET_EMAIL} ha sido cambiada.`);
        console.log(`👉 Tu nueva contraseña es: ${NEW_PASSWORD}`);
        console.log(`Ya puedes iniciar sesión en la aplicación web con esta clave.`);
        
    } catch (error) {
        console.error("❌ Error:", error.message);
    }
}

resetPassword();
